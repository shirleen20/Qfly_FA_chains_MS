...
Combinedplot2 <- DF.Stds %>%
  dplyr::mutate(Class = factor(as.factor(Class), levels = c("DG","TG", "CL", "PC", "PE", "PG", "PI", "PS", "LPC", "PCp","PEp"))) %>%
...
DF.main <- DF %>% 
  mutate(Class2 = if_else(grepl(" e", SubClass), Class, SubClass)) %>% ## All ethyl lipids use ester lipid standards
